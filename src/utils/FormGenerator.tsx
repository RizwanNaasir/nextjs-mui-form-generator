...
export default function useFormGenerator(
  jsonBlueprint: FormBlueprint | undefined
): FormGeneratorResult {
    ...
    setFormValues((prevValues) => {
      if (type === 'checkbox') {
        if (checked) {
          ...
          return {
            ...prevValues,
            [name]: Array.isArray(prevValues[name])
          ...
          return {
            ...prevValues,
            [name]: Array.isArray(prevValues[name])
              ? prevValues[name].filter((v: string) => v !== value)
              : []
          };
        }
      } else {
  ...
  const formElements = jsonBlueprint.fields.map((field, index) => {
    const { type, label, name, xs = 12, options } = field;
