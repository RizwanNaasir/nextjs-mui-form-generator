...
function FormCreator() {
  ...
  const handleAddQuestion = () => {
    ...
    if (!previousQuestionHasLabel && formFields.length !== 0) {
      enqueueSnackbar('Please fill out the previous question', {
        variant: 'warning'
      });
      return;
    }
    ...
    if (formFields.length <= 9) {
      ...
      enqueueSnackbar('You have reached the maximum number of questions', {
        variant: 'error'
      });
    }
  };
  ...
  const handleFormSubmit = async () => {
    ...
    if (totalForms.data().count >= 10) {
      enqueueSnackbar('You have reached the maximum number of forms', {
        variant: 'error'
      });
      setLoading(false);
      return;
    ...
    await addDoc<FormBlueprint>(formBlueprintsRef, newFormBlueprint).then(
      (res) => {
  ...
  function handleQuestionUpdate(questionIndex: number) {
    return (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      updatedFields[questionIndex].name = value
        .replace(/\s+/g, '-')
        .toLowerCase();
